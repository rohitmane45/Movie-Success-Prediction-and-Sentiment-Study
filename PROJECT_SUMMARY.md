...
- **From the TMDB 5000 Movies run** (`python src/main.py --data data/tmdb_5000_movies.csv`):
   - **Sentiment vs Revenue**: -0.016 (very weak / essentially no linear relationship)
...
#### Model Performance
- **From the TMDB 5000 Movies run**:
   - **Linear Regression**: R² = 0.629, MAE ≈ $55.95M
   - **Random Forest**: R² = 0.592, MAE ≈ $56.71M
   - **Best Model Selected**: Linear Regression (higher R²)
...
- Predictions for real movies from the dataset (test set)
- Predictions for hypothetical new movies

Saved artifacts (generated by the pipeline):
- `data/movies_with_sentiment.csv`
- `results/figures/*.png`
- `results/genre_analysis.csv`
- `results/model_comparison.csv`
- `results/model_summary.txt`
- `results/movie_predictions.csv`
